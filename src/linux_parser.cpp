...
long LinuxParser::ActiveJiffies(int pid) { 
  ...
  if (statFileStream.is_open()){
    ...
    totalTime =  stol(data.at(kUtime_ -1)) + stol(data.at(kStime_ -1)) + stol(data.at(kCutime_ -1)) + stol(data.at(kCstime_ -1));
  }
  return totalTime;
}
...
long LinuxParser::ActiveJiffies() { 
  ...
  return (jiffies.at(kUser_) + jiffies.at(kNice_) + jiffies.at(kSystem_) + jiffies.at(kIRQ_) + jiffies.at(kSoftIRQ_) + jiffies.at(kSteal_));
}
...
float LinuxParser::CpuUtilization(int pid){
  ...
  if (statFileStream.is_open()){
    ...
    startTime = stof(data.at(kStarttime_ -1)) / sysconf(_SC_CLK_TCK);
  }
  totalTime = (float)(ActiveJiffies(pid)/ sysconf(_SC_CLK_TCK));
